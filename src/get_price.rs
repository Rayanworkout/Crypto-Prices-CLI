...
impl fmt::Display for GetPriceError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match *self {
            GetPriceError::FetchError(ref err) => {
                write!(f, "An error occured while fetching the API: {err}")
            }
            GetPriceError::JsonDeserialize(ref err) => {
                write!(f, "An error occured while parsing the json: {err}")
            }
        }
    }
}
...
pub async fn get_price(token: &str) -> Result<(), GetPriceError> {
    let url =
        format!("https://api.coingecko.com/api/v3/simple/price?ids={token}&vs_currencies=usd",);
    ...
    let price = match price_object.get(token) {
        Some(token) => match token.get("usd") {
            Some(value) => value,
            None => {
                return Err(GetPriceError::JsonDeserialize(format!(
                    "could not parse the following data: {resp}"
                )));
            }
        },
        None => {
